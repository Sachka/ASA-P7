...
def read_corpus(filename):
    """
    Returns corpus de train pour AVGPerceptron
    """
    sentence = ""
    sentences = []
    ...
    while line:
        ...
        if line == "\n":
            ...
            if RepresentsFloat(word_annotation[1]) or RepresentsInt(word_annotation[1]):
...
def make_dataset(text):
    ...
    for line in text:
        ...
        dataset.extend(zip(pos, zip(tok_trigrams, tok_bigramsL, tok_bigramsR, toks, suff, preff, maj, tags_1))) #maj
...
class AvgPerceptron:
    ...
    def train(self,dataset, dev, step_size=0.1,max_epochs=20):
        ...
        for e in range(max_epochs):
            ...
            #Print loss et acc
            print("Epoch = " + str(e) + ", Loss (#errors) = " + str(loss) + ", Accuracy = " + str(acc * 100) +", Dev acc = " + str(dev_acc * 100))
            #Stop if loss null
            if loss == 0.0:
