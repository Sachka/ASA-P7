...
class LogisticModel:
    ...
    def loglikelihood(self, weights, dataset):
        ...
        for (y, X) in dataset:
            ...
            loglik += log(ysucc + sys.float_info.epsilon) if y else log(1 -
                                                                        ysucc + sys.float_info.epsilon)
        return -loglik
    ...
    def trainBFGS(self, dataset):
        ...
        res = minimize(self.loglikelihood, np.zeros(D), args=(
            dataset,), jac=self.batch_gradient, method="BFGS")
